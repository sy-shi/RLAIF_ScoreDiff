...
def resnet5(in_channels, n_classes):
    return ResNet(in_channels, n_classes, block=ResNetBasicBlock, blocks_sizes=[16, 32, 64], depths=[1])
...
def create_convolutional_layers(model_type, in_channels, out_size):
    model = None

    if model_type == "resnet5":
        model = resnet5(in_channels, out_size)
    elif model_type == "resnet18":
        model = resnet18(in_channels, out_size)
    elif model_type == "resnet34":
        model = resnet34(in_channels, out_size)
    elif model_type == "resnet50":
        model = resnet50(in_channels, out_size)
    elif model_type == "resnet101":
        model = resnet101(in_channels, out_size)
    elif model_type == "resnet152":
        model = resnet152(in_channels, out_size)
    else:
        raise("Invalid resnet type: {}".format(model_type))
