...
class Door(WorldObj):
    ...
    def toggle(self, env, pos):
        ...
        if self.is_locked and env.carrying is not None:
...
class MiniGridEnv(gym.Env):
        ...
        # # Toggle/activate an object
            ...
            if fwd_cell:
                ...
                if fwd_cell.type == 'door' and fwd_cell.is_open:
                    # reward += 0.5 #- 0.9*(self.step_count / self.max_steps)
                    self.grid.set(*fwd_pos, None)
    ...
    def get_obs_render(self, obs, tile_size=CELL_PIXELS//2, mode='pixmap'):
        ...
        if self.obs_render == None:
            from environment.minigrid.rendering import Renderer
    ...
    def render(self, mode='human', close=False, highlight=False, tile_size=CELL_PIXELS):
        """
